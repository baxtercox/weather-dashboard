...
const fetchInfo = (city, isLanding = false) => {
  ...
  const geoURL =
    "https://cors-anywhere.herokuapp.com/http://api.geonames.org/searchJSON?q=";
    ...
    .then(function () {
      ...
      $.ajax({
        ...
        .then(function () {
          ...
          const pixaEndPoint =
            pixaUrl + pixaApiKey + "&q=" + state.city + "&image_type=photo";
    ...
    card.html(
      `
        <div class="card-panel blue">
          <div class="white-text">${new Date(
            state.forecast[i].time * 1000
          ).toDateString()}</div>
          <div><img src="./img/${
            state.forecast[i].icon
          }.png" alt="Weather Icon ${
        state.forecast[i].icon
      }" class="weather-icon"></div>
          <div class="white-text">Temp: ${
            state.forecast[i].temperatureHigh
          }</div>
